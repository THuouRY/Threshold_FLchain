...
def create_public_key_and_shares_centralized(curve_params: CurveParameters,
    ...
    """
    d = number.random_in_range(1, curve_params.order) # this is the secret being shared
    Q = d * curve_params.P
...
def encrypt_message(message: str, public_key: PublicKey) -> EncryptedMessage:
    ...
    encoded_message = bytes(message, 'utf-8') if type(message) == str else bytes(str(message),'utf-8')
